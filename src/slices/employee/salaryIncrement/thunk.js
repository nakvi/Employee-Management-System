...
export const submitSalaryIncrement =createAsyncThunk("salaryIncrement/SubmitSalaryIncrement", async (payload, { rejectWithValue }) => {
  try {
    const response = await fetch(API_ENDPOINT, {
      method: "POST",
      headers: {
    ...
    const data = await response.json();
    toast.success(data.message || "Salary Increment added successfully!");
    return data.data; // Return the newly created Designation
  } catch (error) {
    toast.error("Failed to add Salary Increment. Please try again!");
    return rejectWithValue(error.message);
  }
}
);
...
export const updateSalaryIncrement = createAsyncThunk("salaryIncrement/updateSalaryIncrement",async ( groupData,{ rejectWithValue}) =>{
  try{
    const response= await fetch(`${API_ENDPOINT}`,{
      method : "PUT",
      headers : {
    ...
    toast.success(data.message || "Salary Increment updated successfully!");
    return responseData.data; // Assuming the updated data is in 'data'

  } catch(error) {
    toast.error("Failed to update Salary Increment. Please try again!");
    return rejectWithValue(error.message);
  }
})

// Delete Salary Increment
export const deleteSalaryIncrement =createAsyncThunk("salaryIncrement/deleteSalaryIncrement", async(id, { rejectWithValue}) => {
  try{
    const response =await fetch(`${API_ENDPOINT}`, {
      method: "DELETE",
      headers : {
