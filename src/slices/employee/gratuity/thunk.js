...
const API_ENDPOINT = `${config.api.API_URL}salaryGratuity/`;

// Create the async thunk
export const getGratuity = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      console.error(
        "Error fetching Gratuity :",
        error.message
      );
      // Show error message using toast
      toast.error(
        "Failed to fetch Gratuity. Please try again!"
      );
...
// Submit Gratuity
export const submitGratuity = createAsyncThunk(
  "gratuity/submitGratuity",
  async (payload, { rejectWithValue }) => {
    ...
    try {
      ...
      if (data.status === "0") {
        toast.success(
          typeof data.message === "object"
            ...
            : data.message || "Gratuity added successfully!"
        );
        return data.data;
      ...
      toast.error(
        "Failed to add Gratuity. Please try again!\n" + errorMessage
      );
      return rejectWithValue(errorMessage);
    }
  }
    ...
    try {
      ...
      if (data.status === "0") {
        toast.success(
          typeof data.message === "object"
            ? JSON.stringify(data.message)
            : data.message || "Gratuity updated successfully!"
        );
        return data.data;
      } else if (data.status === "1") {
        const errorMessage =
      ...
      const errorMessage =
        typeof error.message === "object"
          ...
          : error.message || "Unknown error occurred";
      toast.error(
        "Failed to update Gratuity. Please try again!\n" + errorMessage
      );
      return rejectWithValue(errorMessage);
    }
  }
...
export const deleteGratuity = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      ...
      toast.error("Failed to delete Gratuity. Please try again!");
      return rejectWithValue(error.message);
    }
  }
);
