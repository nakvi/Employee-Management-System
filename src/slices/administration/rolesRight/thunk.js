...
// const API_ENDPOINT = "http://192.168.18.65:8001/ems/secPage/";
const API_ENDPOINT = `${config.api.API_URL}secPage/`;

export const getRoleRight = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      toast.error("Failed to fetch Role with permissions. Please try again!");
...
export const submitRoleRight = createAsyncThunk(
  ...
  async (payload, { rejectWithValue }) => {
    ...
    try {
      ...
      const data = await response.json();
      toast.success(data.message || "Role with permissions added successfully!");
      return data.data; // Return the newly created Designation
    } catch (error) {
      toast.error("Failed to add Role with permissions. Please try again!");
      return rejectWithValue(error.message);
    }
  }
);
...
export const updateRoleRight = createAsyncThunk(
  ...
  async (groupData, { rejectWithValue }) => {
    try {
      ...
      toast.success("Role with permissions updated successfully!");
      return responseData.data; // Assuming the updated data is in 'data'
    } catch (error) {
      toast.error("Failed to update Role with permissions. Please try again!");
      return rejectWithValue(error.message);
    }
  }
);
// Delete Grade
export const deleteRoleRight = createAsyncThunk(
  "role/deleteRoleRight",
  async (id, { rejectWithValue }) => {
    try {
      ...
      toast.error("Failed to delete Role with permission. Please try again!");
      return rejectWithValue(error.message);
    }
  }
);