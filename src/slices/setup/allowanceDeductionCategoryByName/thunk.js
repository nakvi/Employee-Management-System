...
const API_ENDPOINT = `${config.api.API_URL}allowDedCatByName/`; 

export const getAllowanceDeductionCategoryByName = createAsyncThunk(
  "allowanceDeductionCategoryByName/getAllowanceDeductionCategoryByName",
  async ({ VType}, { rejectWithValue }) => {
    try {
      const response = await fetch(`${API_ENDPOINT}?Vtype=${VType}`);
      if (!response.ok) {
