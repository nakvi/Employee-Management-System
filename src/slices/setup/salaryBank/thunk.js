...
// const API_ENDPOINT = "http://192.168.18.65:8001/ems/bank/";
const API_ENDPOINT = `${config.api.API_URL}bank/`;

export const getSalaryBank = createAsyncThunk(
  "salaryBank/getSalaryBank",
...
// Submit SalaryBank
export const submitSalaryBank = createAsyncThunk(
  "SalaryBank/submitSalaryBank",
  async (payload, { rejectWithValue }) => {
    try {
      ...
      const data = await response.json();
      toast.success(data.message || "SalaryBank added successfully!");
      return data.data; // Return the newly created Designation
    } catch (error) {
      toast.error("Failed to add SalaryBank. Please try again!");
      return rejectWithValue(error.message);
    }
  }
);
// Update SalaryBank
export const updateSalaryBank = createAsyncThunk(
  "SalaryBank/updateSalaryBank",
  async (groupData, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to update Salary Bank");
      }
      const responseData = await response.json();
      toast.success("Salary Bank updated successfully!");
      return responseData.data; // Assuming the updated data is in 'data'
    } catch (error) {
      toast.error("Failed to update Salary Bank. Please try again!");
      return rejectWithValue(error.message);
    }
  }
);
// Delete SalaryBank
export const deleteSalaryBank = createAsyncThunk(
  "SalaryBank/deleteSalaryBank",
  async (id, { rejectWithValue }) => {
    try {
      ...
      if (responseData.status) {
        toast.success("Salary Bank deleted successfully!");
